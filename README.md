# pi-web-frq

本地运行的多模型 AI coding 工作台：三栏界面，支持项目管理、历史会话、流式对话、文件工作区与 Git。

## 能做什么

- **多模型对话**：选择 Provider 与模型、调节思考强度，SSE 流式输出（含打字机速度显示）。
- **项目与会话**：项目级隔离工作区；会话自动保存、可恢复、重命名、删除、分支续聊。
- **文件工作区**：只读文件树、内容搜索、文件预览（Markdown 可格式化阅读）、Git 状态/差异/提交。
- **多级提示词**：可随时免重启配置全局 `AGENTS.md` + 每项目专属提示词，让模型记得重要事项和行事规则。
- **手机可用**：响应式布局，手机端抽屉式导航。

## 怎么启动

前置条件：Node.js 22+。

### 一键启动（推荐）

```powershell
npm install        # 首次
```

之后**双击 `start.vbs`** 即可：无窗口后台启动服务（日志写 `server.log`），并自动打开浏览器。服务在后台常驻，关闭任何窗口都不影响。

- `start.vbs` / `start.bat`：一键启动（无窗口）
- `stop.bat`：停止服务

首次运行会自动 `npm install` 与生产构建（需要几分钟，进度写在 `start.log`）。

### 手动启动

```powershell
npm install
npm run build      # 生产构建（首次）
npm run start      # 启动服务
```

开发模式：`npm run dev`。

### 访问

- 本机：`http://127.0.0.1:30142`
- 局域网 / Tailscale：`http://本机IP:30142` 或 `http://设备名:30142`

端口和默认工作区可在「全局设置 → 系统」修改，保存到 `service.json`，重启后生效。

## 安全提示

- 服务无登录认证：局域网内设备都能访问，仅限可信网络（本机/局域网）使用。**请勿通过公网穿透（如 Tailscale Funnel、ngrok）直接暴露**。
- 启用的 Plugin 以本机 Node 权限运行，只启用可信来源的插件。
- 模型默认启用读写与 `bash` 工具（cwd 固定在项目根，但命令本身可任意执行，与 pi 内置一致）；如需关闭，设置 `PI_WEB_ALLOW_WRITES=false`。仅限受信任的模型/设备使用。
